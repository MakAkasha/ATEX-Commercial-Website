 ...
 * browser sends for a fetch() from the contact page. The /api CSRF guard
 * (server/middleware/csrf.js) rejects a state-changing request that presents no
 * origin at all, so omitting it would 403 before any validation runs and this
 * file would test the guard instead of the route. Cross-origin and missing-origin
 * behaviour is covered in test/api.csrf.test.js.
 */
const sameOrigin = (srv) => ({ headers: { origin: srv.origin } });
...
describe("POST /api/contact validation", () => {
  ...
  it("a badly formatted whatsapp returns 400 INVALID_WHATSAPP", async () => {
    const res = await srv.post(
      "/api/contact",
      { name: "شركة الاختبار", whatsapp: "0500000004" },
      sameOrigin(srv)
    );
    assert.equal(res.status, 400);
  ...
  it("a name shorter than 2 characters returns 400 INVALID_NAME", async () => {
    const res = await srv.post("/api/contact", { name: "أ", whatsapp: "+966500000005" }, sameOrigin(srv));
    assert.equal(res.status, 400);
  ...
  it("a message under 10 characters returns 400 MESSAGE_TOO_SHORT", async () => {
    const res = await srv.post(
      "/api/contact",
      { name: "شركة الاختبار", whatsapp: "+966500000006", message: "قصير" },
      sameOrigin(srv)
    );
    assert.equal(res.status, 400);
...
describe("POST /api/contact rate limit", () => {
  ...
  it("with CONTACT_RATE_LIMIT_LIMIT=3, five valid submissions produce at least one 429", async () => {
    ...
    for (let i = 0; i < 5; i += 1) {
      const res = await srv.post(
        "/api/contact",
        { ...VALID_BODY, whatsapp: `+96650000100${i}` },
        sameOrigin(srv)
      );
